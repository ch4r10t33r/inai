/**
 * sentrix test — run unit tests for Sentrix agents.
 *
 * Usage:
 *   sentrix test                    # runs all tests in tests/
 *   sentrix test MyAgent            # runs tests/MyAgent.test.ts (or .py/.rs/.zig)
 *   sentrix test --generate MyAgent # scaffold a test file for MyAgent
 *
...
export async function testCommand(
  ...
  if (options.generate) {
    if (!agentName) {
      logger.error('--generate requires an agent name: sentrix test --generate MyAgent');
      process.exit(1);
    }
  ...
  logger.title(`Running Sentrix tests (${lang})`);
...
async function generateTestFile(
  ...
  logger.dim(`Run it with: sentrix test ${agentName}`);
}
 ...
 * Generated by: sentrix test --generate ${agentName}
 */
...
describe('${agentName}', () => {
  ...
  test('responds to __heartbeat capability', async () => {
    const req = {
      ...
      payload:    { senderId: 'sentrix://test', timestamp: Date.now() },
      timestamp:  Date.now(),
    };
    const resp = await agent.handleRequest(req as any);
...
async def test_heartbeat_capability(agent):
    req = AgentRequest(
        ...
        from_id="sentrix://test",
        capability="__heartbeat",
        payload={"senderId": "sentrix://test", "timestamp": 0},
        timestamp=0,
    )
    resp = await agent.handle_request(req)
